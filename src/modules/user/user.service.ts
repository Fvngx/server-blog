...
export class UserService {
  constructor(
    ...
    this.createUser({name, password, role: 'admin'}).then(_ => {
      console.log()
      console.log(`管理员账户创建成功，用户名：${name}, 密码：${password}, 请及时登录系统修改密码`)
      console.log()
    }).catch(_ => {
      console.log()
      console.log(`管理员账户已经存在，用户名：${name}, 密码：${password}, 请及时登录系统修改密码`)
      console.log()
    })
  }
  ...
  async findAll(queryParams: any = {}): Promise<[User[], number]> {
    const query = this.userRepository.createQueryBuilder('user').orderBy('user.create_at', 'DESC')
